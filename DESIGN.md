...
워드마크 옆에 서는 유일한 그림 표식은 **밥그릇 선체에 돛을 단 마크**다 — 황동 그릇 몸통을 유선형 물결 한 줄이 가로지르고, 그 위에 크림 돛 두 장이 뜬다. 밥그릇과 배의 중의(重義)를 한 실루엣에 담았다. **닻과 깃발은 마크가 아니다** — 닻은 재방문·담기, 깃발은 개척의 의미 기호이고 그 분담을 로고가 흐리지 않는다.

파일은 둘이다. `icons/icon.svg`는 딥 네이비 타일이 있는 앱 아이콘이고, `icons/mark.svg`는 타일을 뺀 글리프다. **네이비 면 위에는 글리프, 밝은 면과 OS 컨테이너에는 타일.** 네이비 타일을 네이비 유리 바 위에 또 올리면 카드처럼 떠 보이고, 크림 돛을 크림 위에 맨몸으로 두면 대비가 죽는다.

| 자리 | 파일 | 크기 |
|---|---|---|
| 상단 유리 바, 워드마크 왼쪽 (네 페이지) | `mark.svg` | 28px. ≤419px에서 22px, 워드마크 15px(≤375px 14px), 링크 안쪽 여백 10 → 8px — 로그인 상태의 링크 3개와 한 줄 |
| 로그인 타이틀 타일, 제목 위 | `mark.svg` | 56px. 진입 페이지라 크게 두는 유일한 자리 |
| 푸터, 워드마크 왼쪽 | `icon.svg` | 40px 타일, 곡률 22% — 타일 0·카드 18·사진 12 문법 밖의 앱 아이콘 고유 곡률 |
| 파비콘 · apple-touch-icon · 매니페스트 | `favicon.svg` · `icons/*.png` | OS가 정한다 |

히어로·빈 상태 카드·썸네일 대체·토스트에는 두지 않는다. 히어로는 네이비 면과 큰 제목으로 버티고, 빈 자리는 이니셜이 표시한다.
...
- 가로 스크롤러에는 **`{component.carousel-nav}`를 반드시 짝지운다.** 640px 이상에서만 보이며, 그 아래는 스와이프가 대신한다.
  - **스와이프가 없는 입력에는 컨테이너 포커스로 대신한다.** 스크롤러에 `tabindex="0"` + `role="region"` + `aria-label`을 준다. nav 버튼이 숨는 구간에서 스크롤바까지 `.no-scrollbar`로 지우면, 이게 없는 한 키보드·마우스 드래그 양쪽으로 두 번째 카드 이후에 닿을 방법이 사라진다. (8/28에 `#log-scroller`에서 실제로 그랬다.)
- 섹션 패딩: 80px → 64px → 48px → 40px
- `{component.filter-bar}`: 640px 이상은 상단 sticky, 그 아래는 **타이틀 타일 안 인플로우**(음수 마진으로 히어로의 하단 여백 자리에 앉는다). 스크롤하면 히어로와 함께 올라간다.
  - ~~그 아래는 하단 고정 툴바~~ — **8/24에 폐지했다.** 하단에 상시 고정하면 결과의 약 1/4을 늘 가린다. 되돌리지 말 것. 구현은 `styles.css`의 `#filter-bar`와 `search.html`의 `mt-[-180px]`이 갖는다.
...
| 버전 | 날짜 | 내용 |
|---|---|---|
| beta.4 | 2026-08-25 | **브랜드 마크 도입.** 밥그릇 선체에 돛을 단 앱 아이콘(`icons/icon.svg`)과 타일 없는 글리프(`icons/mark.svg`)를 등록하고 상단 유리 바·로그인 타이틀 타일·푸터·파비콘에 배치했다. 닻 파비콘은 은유 분담(닻=재방문)을 지키기 위해 퇴역. 네이비 면에는 글리프, 밝은 면에는 타일이라는 배치 규칙을 적었다. 420px 미만 워드마크를 18 → 15px(375px 이하 14px)로 내리고 링크 안쪽 여백을 10 → 8px로 조여 마크와 링크 3개가 한 줄에 서게 했다. index 헤더 워드마크를 `<p>`에서 다른 페이지와 같은 `<a>`로 통일했고 푸터 워드마크에 빠져 있던 `font-wordmark`를 붙였다. |
