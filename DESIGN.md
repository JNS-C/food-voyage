...
- 재방문율을 별이 아니라 **닻 5개 + 비율 숫자**로 표기한다. 이 서비스의 시각적 서명이다.
...
Pretendard CDN은 사용 시점의 최신 버전 경로를 확인해서 넣는다.
...
placeholder는 **동네 이름**이다. `성신여대입구역`. 가게명이 아니다. 가게명 검색은 기존 맛집 앱의 문법이고, 동네 검색은 개척 컨셉을 강화한다. 이 구분은 장식이 아니라 서비스 정체성이다.
...
닻 5개를 나란히 두고, 재방문 비율을 5단계로 반올림한 만큼 `{colors.accent}`로 채운다. 나머지는 `{colors.glyph-empty}` 아웃라인. 글리프 16px, 자간 4px. 오른쪽에 실제 비율을 `{typography.ratio-number}`로 병기한다.
...
- 닻 글리프의 소스가 미정이다. 아이콘 라이브러리를 쓸지 직접 SVG로 그릴지 정해야 한다. 16px에서 형태가 읽히는지가 기준이다.
